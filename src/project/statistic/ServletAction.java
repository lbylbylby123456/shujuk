...
public class ServletAction extends HttpServlet {
	...
	public void getFlowStatisticQuerySetup(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JSONObject jsonObj=new JSONObject();
		/*----------------------------------------数据获取完毕，开始返回数值*/
		if(session.getAttribute("project_forum_statistic_query_time_from")!=null){jsonObj.put("time_from",(String)session.getAttribute("project_forum_statistic_query_time_from"));}else{resultCode=1;}
		if(session.getAttribute("project_forum_statistic_query_time_to")!=null){jsonObj.put("time_to",(String)session.getAttribute("project_forum_statistic_query_time_to"));}else{}
		if(session.getAttribute("project_forum_statistic_query_timeinterval")!=null){jsonObj.put("time_interval",(String)session.getAttribute("project_forum_statistic_query_timeinterval"));}else{}
		if(session.getAttribute("project_forum_statistic_query_addressid")!=null){jsonObj.put("address_id",(String)session.getAttribute("project_forum_statistic_query_addressid"));}else{}
