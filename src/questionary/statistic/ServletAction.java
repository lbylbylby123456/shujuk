...
public class ServletAction extends HttpServlet {
	...
	/*----------------------------------*/
	//这里是需要改的,module和sub
	public String module = "questionary";
	...
	public void statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(isAjax){
			...
			resultUrl=resultPath+"/forum_statistic_result.jsp";
	...
	public void getStatisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(existResultset.equals("1")){			//如果是新查询
			...
			if(session.getAttribute(module+"_"+sub+"_statistic_record_result")!=null){
				...
				showDebug("取出了原来的结果"+jsonObj.toString());
			}else{
	...
	public void getFlowStatisticQuerySetup(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JSONObject jsonObj=new JSONObject();
		/*----------------------------------------数据获取完毕，开始返回数值*/
		if(session.getAttribute("questionary_forum_statistic_query_time_from")!=null){jsonObj.put("time_from",(String)session.getAttribute("questionary_forum_statistic_query_time_from"));}else{resultCode=1;}
		if(session.getAttribute("questionary_forum_statistic_query_time_to")!=null){jsonObj.put("time_to",(String)session.getAttribute("questionary_forum_statistic_query_time_to"));}else{}
		if(session.getAttribute("questionary_forum_statistic_query_timeinterval")!=null){jsonObj.put("time_interval",(String)session.getAttribute("questionary_forum_statistic_query_timeinterval"));}else{}
		if(session.getAttribute("questionary_forum_statistic_query_addressid")!=null){jsonObj.put("address_id",(String)session.getAttribute("questionary_forum_statistic_query_addressid"));}else{}
