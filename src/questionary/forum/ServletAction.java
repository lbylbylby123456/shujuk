...
import questionary.dao.forum;


import questionary.dao.forumDao;
...
//		String creator=(String)session.getAttribute("user_name");
//		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
//		String exportFilePathName="C:\\upload\\project\\export\\forum_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
//		JSONObject jsonObj=null;
//		if(existResultset.equals("1")){
...
//		}
		...
		HSSFCell titleCell2= titleRow.createCell(1);           //在行中创建第2个单元格
		titleCell2.setCellValue("评论民宿");                      //设置第2个单元格的值
		HSSFCell titleCell3 =titleRow .createCell(2);          //在行中创建第3个单元格
		titleCell3.setCellValue("评论内容");                      //设置第3个单元格的值
//		HSSFCell titleCell4= titleRow.createCell(3);           //在行中创建第4个单元格
...
//		titleCell6.setCellValue("类型备注");
		...
		for(int i=0;i<jsonArr.length();i++) {
			...
			for (int j = 0; j < 6; j++) {
				...
				Cell2.setCellValue(title);
				HSSFCell Cell3 = valueRow.createCell(2);
				Cell3.setCellValue(content);				HSSFCell Cell4 = valueRow.createCell(3);
//				Cell4.setCellValue(type);
//
				HSSFCell Cell5 = valueRow.createCell(4);              //在第2行中创建单元格
				Cell5.setCellValue(timeFrom);
...
//				Cell6.setCellValue(status);
		...
		HSSFCellStyle cellStyle = wb.createCellStyle();
		wb.write(out);//将响应流输入到Excel表格中
		response.addHeader("Content-Disposition", "attachment;filename=questionary.xls");//文件临时保存在demo.xls中
		out.flush();
		out.close();
