package article.file;
/*
 * 待完成：用MVC模式分开DB和Action操作
 * 增删改查看导印统功能的实现
 */

import article.dao.FileDao;
import article.dao.ArFile;
...
public class ServletAction extends HttpServlet {
	...
	private void downloadRecord(HttpServletRequest request, HttpServletResponse response) throws Exception  {
		...
		response.addHeader("Content-Disposition", "attachment;filename=article.xls");//文件临时保存在demo.xls中
		out.flush();
		out.close();
	}
	/*
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String fileName="xm13_article_file_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		//String exportFilePathName="C:\\upload\\user\\export\\file_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		String exportFilePathName="C:\\upload\\xm13_article_file_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		ArFile query=new ArFile();
		query.setId(id);
		query.setAction(action);
		query.setDbName(dbName);
		...
		if(isAjax){
			...
			redirectUrl="article_list.jsp?exist_resultset=1";
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"index⊙text⊙id","time_interval⊙text⊙userName","count⊙text⊙Email","color⊙text⊙Author","color_name⊙text⊙Tag"};	//name⊙type⊙nick
