...
public class FileDao {
    public JSONObject getRecord(ArFile query) throws SQLException, IOException, JSONException {
        String tableName = "article_file";
        //开始查询数据库
        String resultMsg = "ok";
        int resultCode = 0;
    ...
    public JSONObject modifyRecord(ArFile file) throws JSONException {
        ...
        try {
            ...
            //构造sql语句，根据传递过来的查询条件参数
            String tableName ="article_file";
            String sql = "update " + tableName + " set title='" + file.getTitle() + "',content='" + file.getContent()  + "',author='" + file.getAuthor()  + "',tag='" + file.getTag()  + "',status='" + file.getStatus() + "' where id=" + file.getId();
    ...
    public JSONObject addRecord(ArFile file) throws JSONException, SQLException {
        String resultMsg = "ok";
        int resultCode = 0;
        List jsonList = new ArrayList();
        ylx_db query_db = new ylx_db(file.getDbName());
        //构造sql语句，根据传递过来的查询条件参数
        String tableName = "article_file";
        String sql = "insert into " + tableName + "(parent_id,title,content,author,tag,limit_time,status,user_id,creator,create_time) values('" + file.getParentId() + "','" + file.getTitle() + "','" + file.getContent() +
                "','" + file.getAuthor() + "','" + file.getTag() + "','" + file.getLimitTime() + "','" + file.getStatus() + "','" + file.getUserId() + "','" + file.getCreator() + "','" + file.getCreateTime() + "')";
    ...
    public JSONObject setRecordTop(String action, String dbName, String type, String userId, String id) throws JSONException, SQLException {
        ...
        //构造sql语句，根据传递过来的查询条件参数
        String sql = "select max(priority) as priority from article_file where user_id='" + userId + "'";
        int priority = 0;
        ResultSet rs = query_db.executeQuery(sql);
        if (rs.next()) {
        ...
        query_db.close();
        update_db.executeUpdate("update article_file set priority=" + (priority + 1) + " where id=" + id);
        update_db.close();
        //下面开始构建返回的json
        JSONObject jsonObj = new JSONObject();
        jsonObj.put("aaData", jsonList);
	 ...
	 */
    private String createGetRecordSql(ArFile query) {
        String sql = "";
        String where = "";
