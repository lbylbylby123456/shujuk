...
//			String tableName="order_statistic";
...
//	}
	...
	public JSONObject statisticRecord(String action,String dbName,Statistic statistic) throws SQLException, IOException, JSONException {
		...
		try {
			...
			ResultSet rs = query_db.executeQuery(sql);
            String content=statistic.getContent();
			while (rs.next()) {
	...
	private String getStatisticRecordSql(Statistic statistic){
		String sql="";
		String timeInterval="";
        String type=statistic.getStatisticType();
        String content=statistic.getContent();
		...
		sql="select date_format(create_time,\""+timeInterval+"\") as time_interval,count(*) as count from order_file";
