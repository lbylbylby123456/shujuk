...
public class OrderDao {
	 ...
	 * 功能：返回结果集
	 */
    public JSONObject getRecord(Order query) throws SQLException, IOException, JSONException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                list.add(rs.getString("status"));//9
                list.add(rs.getString("user_id"));//10
    ...
    public JSONObject modifyRecord(Order Order) throws JSONException {
    ...
    public JSONObject addRecord(Order Order) throws JSONException, SQLException {
        String resultMsg = "ok";
        int resultCode = 0;
        List jsonList = new ArrayList();
        ylx_db query_db = new ylx_db(Order.getDbName());
    ...
    public JSONObject setRecordTop(String action, String dbName, String type, String userId, String id) throws JSONException, SQLException {
        ...
        //构造sql语句，根据传递过来的查询条件参数
        String sql = "select max(priority) as priority from order_file where user_id='" + userId + "'";
        int priority = 0;
        ResultSet rs = query_db.executeQuery(sql);
        if (rs.next()) {
        ...
        query_db.close();
        update_db.executeUpdate("update order_file set priority=" + (priority + 1) + " where id=" + id);
        update_db.close();
        //下面开始构建返回的json
        JSONObject jsonObj = new JSONObject();
        jsonObj.put("aaData", jsonList);
	 ...
	 */
    private String createGetRecordSql(Order query) {
        String sql = "";
        String where = "";
