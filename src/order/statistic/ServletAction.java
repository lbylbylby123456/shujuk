...
import order.dao.Statistic;
import order.dao.StatisticDao;
...
public class ServletAction extends HttpServlet {
	...
	//	String addressId=request.getParameter("address_id");
		...
		if(isAjax){
			...
			resultUrl=resultPath+"/order_statistic_result.jsp";
	...
	public void getFlowStatisticQuerySetup(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JSONObject jsonObj=new JSONObject();
		/*----------------------------------------数据获取完毕，开始返回数值*/
		if(session.getAttribute("order_statistic_query_time_from")!=null){jsonObj.put("time_from",(String)session.getAttribute("order_statistic_query_time_from"));}else{resultCode=1;}
		if(session.getAttribute("order_statistic_query_time_to")!=null){jsonObj.put("time_to",(String)session.getAttribute("order_statistic_query_time_to"));}else{}
		if(session.getAttribute("order_statistic_query_timeinterval")!=null){jsonObj.put("time_interval",(String)session.getAttribute("order_statistic_query_timeinterval"));}else{}
		if(session.getAttribute("order_statistic_query_addressid")!=null){jsonObj.put("address_id",(String)session.getAttribute("order_statistic_query_addressid"));}else{}
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        titleCell1.setCellValue("时间段");               //设置第1个单元格的值
        ...
        response.addHeader("Content-Disposition", "attachment;filename=order_statistic_result.xls");//文件临时保存在demo.xls中
        out.flush();
        out.close();
	}
	/*
