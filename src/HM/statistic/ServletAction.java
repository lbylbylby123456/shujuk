package HM.statistic;
/*
 * 增删改查看导印统功能的实现
 * 待完成：用MVC模式分开DB和Action操作
 */

import HM.dao.Statistic;
import HM.dao.StatisticDao;
...
//		String fileName=getStatisticGraph(session,dbName,jsonObj,"待办事项统计图","测试Column",statisticType,1,1280,768);
		...
		if(isAjax){
			...
			int resultCode=0;
			redirectUrl="../hm/hm_list.jsp";
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
			resultUrl=resultPath+"/hm_statistic_result.jsp";
...
//			session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
	...
	public String getStatisticGraph(HttpSession session, String dbName, JSONObject json, String title, String column, String statisticType, int statisticImage, int chartWidth, int chartHeight) throws SQLException, IOException, JSONException {
    	...
    	String chartFilename ="hm_static_" +(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".jpg";
    	String saveSrc="E:\\stat\\"+chartFilename;
    	showDebug(saveSrc);
        try { 

        	fos_jpg = new FileOutputStream(saveSrc); 
            ChartUtilities.writeChartAsJPEG(fos_jpg, 1.0f,chart,chartWidth,chartHeight,null);
        } finally { 
            try { 
                fos_jpg.close(); 
            } catch (Exception e) {} 
