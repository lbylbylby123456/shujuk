...
var Record = function() {
	...
	var exportRecord1=function(){
		if(confirm("导出之前，必须在指定的分区创建对应的目录，否则导出会出错！\r\n请在导出前确保目录C:\\upload\\project\\export存在，如果没有就创建一个。\r\n请问条件符合了吗？")){
			window.location.href="../../"+module+"_"+sub+"_servlet_action?action=export1_record";
...
var Page = function() {
	...
	//	$('#addWJ_button').click(function() {Page.addWJ();});
		...
		$('#export1_button').click(function() {Page.exportRecord1();});
		...
		//$('#print1_button').click(function() {Page.print1Record();});
	};
	...
	var help=function(){
		...
		window.open('../forum/forum_print1.jsp?page_url='+strUrl, 'big', 'fullscreen=yes');
	}
