...
var Page = function() {
	...
	var isInvalidInput=function(str){
	...
	var submitRecord=function()
	{
		console.log("exe111");
		//if(checkInput()==true){
		var prov_val = $("#prov option:selected").val();
		var city_val = $("#city option:selected").val();
		var district_val = $("#country option:selected").val();
		var name = $("#name").val();
		...
		if(prov_val === null ||timefrom===null||timeto===null|| city_val === null || district_val === null || isNaN(price) || isInvalidInput(name) || isInvalidInput(address))
		{
			alert("请填写正确的格式并且内容完整");
		}
		else
		{
			...
			var district = $("#country option:selected").text();
