...
var Record = function() {
	...
	var checkPass = function(id){
		if(confirm("您确定要通过这条审信息核吗？")){
			if(id>-1){
				$.post("../../"+module+"_"+sub+"_servlet_action?action=check_record&op=0&id="+id,function(json){
					if(json.result_code==0){
						var count=json.count;
						var amount=json.amount;
						initRecordList();
	...
	var checkFail = function(id){
		if(confirm("您确定不通过这条审信息核吗？")){
			if(id>-1){
				$.post("../../"+module+"_"+sub+"_servlet_action?action=check_record&op=1&id="+id,function(json){
	...
	return {
		...
		checkPass:function(id){
			checkPass(id);
		},
		checkFail:function(id){
			checkFail(id);
		}
	};
}();//Record;
...
var Page = function() {
	...
	var addRecord=function(){
		window.location.href="CM_add.jsp";
	}
	...
	var showRecord = function(json){
		...
		html=html+"																<span>申请审核时间："+createTime+"</span><p>";
		if(status === "未审核"){
			html=html+"																<button  type=\"button\" class=\"btn-small\" onclick=\"Page.checkPass("+id+");\">通过</button>";
			html=html+"																<button  type=\"button\" class=\"btn-small\" onclick=\"Page.checkFail("+id+");\">不通过</button>";
		}
		else{
	...
	var modifyRecord=function(id){
		window.location.href="CM_view.jsp?id="+id;
	};
	var searchRecord=function(){
		window.location.href="CM_query.jsp";
	};
	var statisticRecord=function(){
		window.location.href="CM_statistic.jsp";
	}
...
//		if(layout==2)
	...
	var printRecord=function(){
		window.location.href="CM_print.jsp?exist_resultset=1";
	};
