...
var Record = function() {
	...
	//	Metronic.startPageLoading({message: '正在查询中，请稍候...'});	//开始等待动画
		...
		var url="../../"+module+"_"+sub+"_servlet_action?action=get_statistic_record&id="+id+"&exist_resultset="+existResultset;
...
var Page = function() {
	...
	var checkInput=function(){
        ...
        if(statistic_type==null || statistic_type==""){
            Frame.showMsg("统计内容不能为空！");bOk=false;
        }else if(content==null || content==""){
            Frame.showMsg("统计内容不能为空！");bOk=false;
        }else if(beginTime==null || beginTime==""){
            Frame.showMsg("开始时间不能为空！");bOk=false;
        }else if(endTime==null || endTime==""){
            Frame.showMsg("结束时间不能为空！");bOk=false;
        }
		return bOk;
	};
	var deleteRecord=function(id){
