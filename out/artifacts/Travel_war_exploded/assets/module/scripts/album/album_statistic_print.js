...
var Record = function() {
	...
	var showStatisticList=function(json){
		...
		html=html+"序号----时间段----总数";
		for(var i=0;i<list.length;i++){
			html=html+"<li>"+list[i][0]+"----"+list[i][1]+","+list[i][2]+"，统计类型："+list[i][3]+"</li>";
		}
		html=html+"</ul>";
					...
					if(json.result_code==0){
						...
						if(list!=null && list.length>0){
							if(confirm("您当前有 "+list.length+" 项未完成导出工作，继续进行新的导出吗？")){
								window.location.href="../xm13-minsu/"+module+"_"+sub+"_servlet_action?action=export_statistic_resultset&export_type="+exportType+"&exist_resultset=1";
							}else{
