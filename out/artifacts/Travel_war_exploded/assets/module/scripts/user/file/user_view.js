...
var Record = function() {
	...
	var getRecordView = function(url){
		console.log("hello I am in getRecordView");
...
var Page = function() {
	...
	var showResult=function(json){
		console.log("HELLO I AM IN showResult");
		var title="记录显示";
	...
	var showRecordList=function(list){
		console.log("i am in showRecordlist");
	...
	var showRecord = function(json){
		...
		var gender=json[3];
		var city=json[4];
		console.log("hello,i am in showRecord");
	...
	var checkInput=function(){
		...
		if(action==null || action==""){
			Frame.showMsg("名称不能为空！");bOk=false;
		}else if(title==null || title==""){
			Frame.showMsg("职位名不能为空！");bOk=false;
		}else if(content==null || content=="") {
			Frame.showMsg("职位邮箱不能为空！");
			bOk = false;
		}
		else if(!myreg.test(String(content))){
			Frame.showMsg("请再检查一下邮箱格式哦！");bOk=false;
		}
		//Frame.showMsg("bok="+bOk);
		return bOk;
		/*else if(gender==null || gender==""){
		 Frame.showMsg("性别不能为空！");bOk=false;
		 }else if(city==null || city==""){
		 Frame.showMsg("城市不能为空！");bOk=false;
		 }*/
	};
	var deleteRecord=function(id){
		id=$("#id").val();
