...
var Record = function() {
	...
	var initRecordListHeader = function() {
		...
		html=html+"							<th>民宿名称</th>";
		html=html+"							<th>地址</th>";
		html=html+"							<th>价格/晚</th>";
		html=html+"							<th>添加时间</th>";
		html=html+"							<th>状态</th>";
		html=html+"						</tr>";
				...
				$.ajax({
					...
					"success": function(json){
						...
						if(json.result_code==0){
							...
							console.log("_______JSON_________");
							...
							if(list.length>0){
								...
								for(var i = 0; i < list.length;i++) {
									var oneRow = [];
									oneRow.push(list[i][0]);
									oneRow.push(list[i][1]);
									oneRow.push(list[i][2]);
									oneRow.push(list[i][3]);
									oneRow.push(list[i][7]);
									oneRow.push(list[i][4]);
									data.push(oneRow);
								}
								//temp.aaData = data;
								jsonObj.aaData =data;

								console.log("_______NEW___JSON_________");
	...
	var addNew=function(){
		var title=encodeURI(document.getElementById("name").value);
		var content=encodeURI(document.getElementById("address").value);
