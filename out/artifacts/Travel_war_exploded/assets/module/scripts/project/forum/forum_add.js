...
var Page = function() {
	...
	var initPageStyle = function() {
		$('#create_time').datetimepicker({
			...
			$('#limit_time').datetimepicker('setStartDate',startTime);
		});
		$('#limit_time').datetimepicker({
			Date:'yyyy-mm-dd',
			format: 'yyyy-mm-dd',
			language:'zh-CN',
			autoclose:true,
			...
			var endTime = e.date;
			$('#create_time').datetimepicker('setEndDate',endTime);
		});
		//新加入的代码
		hideFrameNav();
	};
	...
	var checkInput=function(){
		...
		//var a=document.getElementById('userConnect').value;
		if(title==null || title==""){
			Frame.showMsg("标题不能为空！");bOk=false;
		}else if(content==null || content==""){
			Frame.showMsg("内容不能为空！");bOk=false;
		}
			// else if(userConnect==null || userConnect==""){
		//	Frame.showMsg("联系方式不能为空！");bOk=false;
		//}
		return bOk;
	};
	var deleteRecord=function(id){
